...
class ColorRGBA(object):
    ...
    @classmethod
    def from_pallete(cls, color_name):

        col_values = getattr(Pallete, color_name, None)
        if col_values is None:
            raise ValueError( "Unknown color name (%s)" % color_name )
        return cls(col_values)
...
class _Palette:
    ...
    def __init__(self):
        self.__class__.__dict__["colors"] = \
            dict (
    ...
    def __getattr__(self, color_name):
        try:
            return ColorRGBA(self.colors[color_name])
        except KeyError:
            raise AtrributeError("Unknown color (%s)"%color_name)

    def __setattr__(self, color_name, value):
        raise TypeError("Palette can not be modified")

    def __getitem__(self, color_name):
        try:
            return ColorRGBA(self.colors[color_name])
        except KeyError:
            raise KeyError("Unknown color (%s)"%color_name)
...
if __name__ == "__main__":
    ...
    print palette.red + palette.blue
    print palette.red.rgb8
